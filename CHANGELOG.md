...
- Fixed cross-session resume dropping many completed epochs. Two causes: (1) resume loaded the
  Drive checkpoint even when a fresher LOCAL one existed; it now loads the checkpoint with the
  highest completed epoch. (2) The ~250 MB Drive checkpoint (dominated by Adam optimizer state)
  often had not finished uploading to Google when Colab reset, so progress.json's
  recoverable_epoch over-reported. The Drive copy now omits the optimizer state (~1/3 the size,
  syncs reliably); on a cross-session resume the optimizer is re-initialized (Adam re-warms in a
  few steps). LOCAL keeps the full checkpoint for exact same-session resume.
